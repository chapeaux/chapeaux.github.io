...
**Navigation helpers.** `pages()` filters and sorts pages by any frontmatter field, with a `depth` parameter for hierarchical sections — `pages(section="foundations", depth=1)` returns only direct children, not grandchildren. `tree()` builds full navigation trees from the <abbr title="Uniform Resource Locator">URL</abbr> structure. Both replace the need for <abbr title="SPARQL Protocol and RDF Query Language">SPARQL</abbr> in common navigation patterns. Auto-generated `urn:geoff:meta:depth` and `urn:geoff:meta:parent` triples make hierarchy queries trivial in <abbr title="SPARQL Protocol and RDF Query Language">SPARQL</abbr> too.
